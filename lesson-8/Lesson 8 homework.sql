...
select Category, count(ProductName) as [Number of products] 
from products
group by Category
--Question 2
Select Category, avg(Price) as [averagepr] from products where Category='Electronics' GROUP BY Category
--Question 3
...
--Question 5
Select CustomerID, City from Customers where City like'%A'
--Question 6
-1
Select Max(Price) as [Qimmat] from Products
--2
...
--Question 9
select*from orders
Select min(Quantity) as [minquan], max(quantity) as [maxquan] from orders
--Question 10
Select distinct CustomerID from Orders
where OrderDate>='01.01.2023' and orderdate<'01.02.2023' 
and customerid not in (select distinct customerID FROM orders)
--Question 11
select * from products select * from products_discounted
...
Select ProductName from Products_Discounted
--Question 13
select Year(OrderDate) as year, avg(quantity) as [avquan] from orders
...
when price >500 then 'high' end as pricegroups from products
--Question 15
...
--Question 16
Select ProductID, Sum(saleamount) as total from sales group by ProductID
--Question 17
select*from Products
Select Productname from products where ProductName like '%oo%' 
--Question 18
select * from city_population
select year, [Bektemir], [Chilonzor], [Yakkasaroy] into Pop_Each_City from city_population 
PIVOT
(
...
unpivot (population for year in ([2012], [2013])) as unpivot_table

--Question 21
select Products.ProductName,
Count (sales.SaleID) AS timessold
from Products
JOIN Sales ON Products.ProductID = Sales.ProductID
GROUP BY Products.ProductName
ORDER BY TimesSold DESC
--Question 22
select district_name, population, year from Pop_Each_City
unpivot (population for district_name in ([Bektemir], [Chilonzor], [Yakkasaroy])) as unpivot_table
